...
from tests import obs_contents, soft_delete_store
...
class TestEnsureProjectRoot:
    def test_creates_root_entity(self, store: Storage) -> None:
        _ensure_project_root(store, "my-project")
        entity = store.reads.get_entity("my-project", "project/my-project")
        assert entity.entity_type == "project"

    def test_idempotent(self, store: Storage) -> None:
        _ensure_project_root(store, "my-project")
        _ensure_project_root(store, "my-project")
        entity = store.reads.get_entity("my-project", "project/my-project")
        assert entity.entity_type == "project"

    def test_skips_global(self, store: Storage) -> None:
        _ensure_project_root(store, "global")
        with pytest.raises(ValueError, match="not found"):
            store.reads.get_entity("global", "project/global")
...
class TestScopeUniqueness:
    def test_rejects_project_entity_in_global_if_exists_in_project(self, store: Storage) -> None:
        entity: list[dict[str, object]] = [{"name": "task/abc", "entityType": "task", "observations": ["x"]}]
        store.entities.create("my-proj", entity)
        assert store.entities.exists_in("task/abc", "my-proj")
        conflict = store.entities.exists_outside("task/abc", _GLOBAL_PROJECT)
        assert conflict == "my-proj"

    def test_allows_same_name_in_two_project_scopes(self, store: Storage) -> None:
        entity: list[dict[str, object]] = [{"name": "task/abc", "entityType": "task", "observations": ["x"]}]
        store.entities.create("proj-a", entity)
        assert store.entities.exists_outside("task/abc", "proj-b") == "proj-a"
        # But the check only applies to global, so proj-b should be allowed
        assert not store.entities.exists_in("task/abc", _GLOBAL_PROJECT)
...
class TestRelationTypeWarnings:
    def _seed_nonconforming(self, db: Storage) -> None:
        db.entities.create(
            "proj",
            [
        ...
        with db.connection.transaction():
            db.connection.write("INSERT OR IGNORE INTO relation_types (name) VALUES ('legacy_thing')")
            src = db.connection.query_one("SELECT id FROM entities WHERE name = 'a'")[0]
            tgt = db.connection.query_one("SELECT id FROM entities WHERE name = 'b'")[0]
            type_id = db.connection.query_one("SELECT id FROM relation_types WHERE name = 'legacy_thing'")[0]
            db.connection.write(
                "INSERT INTO relations (source_id, target_id, relation_type_id) VALUES (?, ?, ?)",
                (src, tgt, type_id),
            )

    def test_warning_present_for_nonconforming(self, server_db: Storage) -> None:
    ...
    def test_warning_present_for_grouped_all_projects_search(self, server_db: Storage) -> None:
        self._seed_nonconforming(server_db)
        result = server.search_all_projects("x")
        assert "legacy_thing" in result["relationTypeWarnings"]

    def test_no_warning_for_clean_data(self, server_db: Storage) -> None:
...
class TestGetEntityWithRelationsFilters:
    ...
    def test_filter_by_entity_type(self, server_db: Storage) -> None:
        self._seed(server_db)
        result = server.get_entity_with_relations("proj", "a", entityType="project")
        related_names = {e["name"] for e in result["relatedEntities"]}
        assert related_names == {"b"}

    def test_filter_by_relation_type(self, server_db: Storage) -> None:
        self._seed(server_db)
            ...
            [
                {
                    ...
                    "entityType": "task",
                    "observations": ["obs"],
                    "relations": [{"source": "other/entity", "target": "project/foo", "type": "belongs-to"}],
                }
            ],
        )
...
class TestRuntimePolicyErrors:
    def test_create_relations_returns_error_for_task_project_belongs_to(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
    ...
    def test_create_entities_returns_error_for_strict_project_scoped_user_preferences(
        self, monkeypatch: pytest.MonkeyPatch, server_db: Storage
    ) -> None:
...
class TestListProjects:
    def test_returns_all_projects(self, store: Storage) -> None:
        store.entities.create("alpha", [{"name": "e1", "entityType": "project", "observations": ["a"]}])
        store.entities.create("beta", [{"name": "e2", "entityType": "project", "observations": ["b"]}])
        assert store.projects.names() == ["alpha", "beta"]

    def test_empty_database(self, store: Storage) -> None:
        assert store.projects.names() == []


class TestSetEntityStatusTool:
    def test_bloat_warning_when_resolved_over_ceiling(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a", "b", "c", "d"]}])
        result = server.set_entity_status("proj", "e1", "resolved")
        assert "bloatWarning" in result

    def test_no_bloat_warning_at_or_below_ceiling(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a", "b", "c"]}])
        result = server.set_entity_status("proj", "e1", "resolved")
        assert "bloatWarning" not in result

    def test_no_bloat_warning_for_non_resolved_status(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a", "b", "c", "d"]}])
        result = server.set_entity_status("proj", "e1", "in-progress")
        assert "bloatWarning" not in result

    def test_missing_entity_returns_error(self, server_db: Storage) -> None:
        assert "error" in server.set_entity_status("proj", "nope", "resolved")


class TestVoteEntityTool:
    def test_valid_vote_returns_new_score(self, server_db: Storage) -> None:
    ...
    def test_invalid_vote_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
...
class TestVoteObservationTool:
    def test_valid_vote_by_content_returns_new_score(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
    ...
    def test_valid_vote_by_hash(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
        content_hash = server_db.reads.get_entity("proj", "e1").observations[0].content_hash
    ...
    def test_neither_addressing_votes_entity(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
        assert server.vote("proj", "e1", 1) == {
            "name": "e1",
            "project": "proj",
    ...
    def test_both_addressing_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
        content_hash = server_db.reads.get_entity("proj", "e1").observations[0].content_hash
        assert "error" in server.vote("proj", "e1", 1, observation="x", observationHash=content_hash)

    def test_invalid_vote_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
        assert "error" in server.vote("proj", "e1", 5, observation="x")

    def test_missing_observation_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
        assert "error" in server.vote("proj", "e1", 1, observation="nope")


class TestAddObservationsTool:
    def test_returns_hashes_of_new_observations(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a"]}])
        result = server.add_observations("proj", "e1", ["b", "c"])
        assert result["count"] == 2
        assert result["hashes"] == [hash_observation("b"), hash_observation("c")]


class TestDeleteObservationsTool:
    def test_delete_by_content(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a", "b"]}])
        result = server.delete_observations("proj", "e1", observations=["a"])
        assert result["count"] == 1
        assert obs_contents(server_db.reads.get_entity("proj", "e1")) == ["b"]

    def test_delete_by_hash(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a", "b"]}])
        result = server.delete_observations("proj", "e1", observationHashes=[hash_observation("a")])
        assert result["count"] == 1
        assert obs_contents(server_db.reads.get_entity("proj", "e1")) == ["b"]


class TestTrimObservationsToOutcomeTool:
    def test_trim_returns_deleted_count(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a", "b", "c"]}])
        result = server.trim_observations_to_outcome("proj", "e1", [hash_observation("a")])
        assert result == {"message": "Trimmed 2 observation(s) from 'e1'.", "deleted": 2}
        assert obs_contents(server_db.reads.get_entity("proj", "e1")) == ["a"]

    def test_empty_keep_hashes_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a"]}])
...
class TestBulkRenameEntityTool:
    def test_rename_returns_message(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "a", "entityType": "task", "observations": ["x"]}])
        result = server.rename_entity("proj", "a", "a2")
        assert "message" in result
        assert server_db.entities.exists_in("a2", "proj")

    def test_collision_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
                {"name": "a", "entityType": "task", "observations": ["x"]},
...
class TestMoveEntityCrossScopeTool:
    def test_move_returns_dropped_relations(self, server_db: Storage) -> None:
        ...
        server_db.relations.create("src", [Relation(source="a", target="b", relation_type="belongs-to")])
        result = server.move_entity_cross_scope("src", "dst", "a")
        assert result["droppedRelations"] == [{"source": "a", "target": "b", "relation_type": "belongs-to"}]
        assert server_db.entities.exists_in("a", "dst")

    def test_target_collision_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("src", [{"name": "e1", "entityType": "task", "observations": ["x"]}])
        server_db.entities.create("dst", [{"name": "e1", "entityType": "task", "observations": ["y"]}])
...
class TestSearchNodesObservationShape:
    def test_search_output_carries_content_hash_per_observation(self, server_db: Storage) -> None:
        ...
        assert observation["content_hash"] == hash_observation("needle")
...
class TestMergeEntitiesTool:
    def test_merge_reports_counts_and_folds_source(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
        ...
        assert "from-source" in obs_contents(server_db.reads.get_entity("proj", "canon"))
        with pytest.raises(ValueError, match="not found"):
            server_db.reads.get_entity("proj", "dup")

    def test_merge_into_self_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["a"]}])
        assert "error" in server.merge_entities("proj", "e1", "e1")

    def test_missing_entity_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "canon", "entityType": "task", "observations": ["b"]}])
        assert "error" in server.merge_entities("proj", "nope", "canon")


class TestMergeObservationsTool:
    def test_merge_reports_count_and_removes_source(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["source", "target"]}])
        entity = server_db.reads.get_entity("proj", "e1")
        ...
        assert obs_contents(server_db.reads.get_entity("proj", "e1")) == ["target"]

    def test_unknown_hash_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "e1", "entityType": "task", "observations": ["t"]}])
        target = server_db.reads.get_entity("proj", "e1").observations[0].content_hash
        assert "error" in server.merge_observations("proj", "e1", "deadbeef", target)


class TestProjectPathTools:
    def test_set_project_paths_registers_and_creates_root(self, server_db: Storage, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        result = server.set_metadata("platform", "paths", [str(repo)])
        assert result["paths"]
        assert server_db.reads.get_entity("platform", "project/platform").entity_type == "project"

    def test_set_project_paths_returns_only_own_paths(self, server_db: Storage, tmp_path: Path) -> None:
        repo_a = tmp_path / "a"
        repo_b = tmp_path / "b"
        repo_a.mkdir()
        repo_b.mkdir()
    ...
    def test_get_project_for_path_hit_and_miss(self, server_db: Storage, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
    ...
    def test_list_project_paths_returns_mappings(self, server_db: Storage, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        server.set_metadata("platform", "paths", [str(repo)])
    ...
    def test_duplicate_path_returns_error(self, server_db: Storage, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        server.set_metadata("platform", "paths", [str(repo)])
    ...
    def test_delete_project_removes_empty_scope(self, server_db: Storage, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        server.set_metadata("doomed", "paths", [str(repo)])
        server_db.entities.delete("doomed", "project/doomed")
        assert server.delete_project("doomed") == {"message": "Deleted project 'doomed'."}
        assert "doomed" not in server_db.projects.names()

    def test_delete_project_non_empty_returns_error(self, server_db: Storage) -> None:
        server.set_metadata("busy", "paths", [])
        result = server.delete_project("busy")
        assert "error" in result

    def test_move_project_entities(self, server_db: Storage) -> None:
        server_db.entities.create("src", [{"name": "e1", "entityType": "task", "observations": ["a"]}])
        server_db.entities.create("dst", [{"name": "d0", "entityType": "task", "observations": ["x"]}])
        assert server.move_project_entities("src", "dst") == {
            "message": "Moved 1 entities from 'src' to 'dst'.",
            "moved": 1,
        }
        assert obs_contents(server_db.reads.get_entity("dst", "e1")) == ["a"]

    def test_move_project_entities_collision_returns_error(self, server_db: Storage) -> None:
        server_db.entities.create("src", [{"name": "dup", "entityType": "task", "observations": ["a"]}])
        server_db.entities.create("dst", [{"name": "dup", "entityType": "task", "observations": ["b"]}])
        assert "error" in server.move_project_entities("src", "dst")

    def test_get_paths_for_project_hit_and_miss(self, server_db: Storage, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        server.set_metadata("platform", "paths", [str(repo)])
...
class TestProjectGroupTools:
    def test_set_project_groups_registers_and_creates_root(self, server_db: Storage) -> None:
        result = server.set_metadata("platform", "groups", ["tooling"])
        assert result == {"project": "platform", "members": []}
        assert server_db.reads.get_entity("platform", "project/platform").entity_type == "project"

    def test_set_project_groups_returns_siblings(self, server_db: Storage) -> None:
    ...
    def test_list_project_groups_returns_mappings(self, server_db: Storage) -> None:
    ...
    def test_get_group_members_hit_and_miss(self, server_db: Storage) -> None:
        server.set_metadata("first", "groups", ["tooling"])
...
class TestMetadataToolInvalidKind:
    def test_list_metadata_invalid_kind_returns_error(self, server_db: Storage) -> None:
        assert "error" in server.list_metadata("bogus")

    def test_set_metadata_invalid_kind_returns_error(self, server_db: Storage) -> None:
        assert "error" in server.set_metadata("platform", "bogus", ["x"])


class TestSearchTools:
    def test_search_nodes_match_all_narrows_results(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
    ...
    def test_search_all_projects_match_all_narrows_results(self, server_db: Storage) -> None:
        server_db.entities.create("p1", [{"name": "a", "entityType": "task", "observations": ["alpha only"]}])
        server_db.entities.create("p2", [{"name": "both", "entityType": "task", "observations": ["alpha beta"]}])
    ...
    def test_search_nodes_status_list_ors_statuses(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
    ...
    def test_search_all_projects_projects_filter_narrows_to_named_projects(self, server_db: Storage) -> None:
        server_db.entities.create("p1", [{"name": "a", "entityType": "task", "observations": ["hello"]}])
        server_db.entities.create("p2", [{"name": "b", "entityType": "task", "observations": ["hello"]}])
        server_db.entities.create("p3", [{"name": "c", "entityType": "task", "observations": ["hello"]}])
        result = server.search_all_projects("hello", projects=["p1", "p2"])["results"]
        assert set(result) == {"p1", "p2"}

    def test_search_all_projects_expand_groups_unions_siblings(self, server_db: Storage) -> None:
    ...
    def test_search_all_projects_expand_groups_without_projects_errors(self, server_db: Storage) -> None:
        assert "error" in server.search_all_projects("hello", expand_groups=True)

    def test_search_all_projects_returns_each_relation_only_under_its_project(self, server_db: Storage) -> None:
        server_db.entities.create(
            "p1",
            [
    ...
    def test_search_all_projects_groups_relations_by_owning_project(self, server_db: Storage) -> None:
        for proj in ("aaa", "zzz"):
            server_db.entities.create(
                proj,
                [
    ...
    def test_search_nodes_forwards_include_archived(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
                {
    ...
    def test_search_all_projects_forwards_include_archived(self, server_db: Storage) -> None:
        server_db.entities.create(
            "proj",
            [
                {
...
class TestSearchToolsObservationBudget:
    ...
    def test_search_nodes_negative_returns_all(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.search_nodes("proj", "foo", max_observation_chars=-1)["entities"][0]
        assert obs_contents(entity) == ["aaa", "bbb", "ccc"]

    def test_omitted_count_replaces_sentinel_observation(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.search_nodes("proj", "foo", max_observation_chars=0)["entities"][0]
        assert obs_contents(entity) == ["aaa"]
        assert entity["omitted"] == 2

    def test_uninformative_fields_are_omitted(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.search_nodes("proj", "foo", max_observation_chars=-1)["entities"][0]
        assert "vote_score" not in entity
        assert "status" not in entity
        assert "omitted" not in entity
        assert all("vote_score" not in o for o in entity["observations"])

    def test_search_nodes_small_budget_trims_with_sentinel(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.search_nodes("proj", "foo", max_observation_chars=6)["entities"][0]
        assert obs_contents(entity) == ["aaa", "bbb"]

    def test_search_nodes_zero_keeps_only_top(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.search_nodes("proj", "foo", max_observation_chars=0)["entities"][0]
        assert obs_contents(entity) == ["aaa"]

    def test_read_graph_small_budget_trims_with_sentinel(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.read_graph("proj", max_observation_chars=6)["entities"][0]
        assert obs_contents(entity) == ["aaa", "bbb"]

    def test_read_graph_negative_returns_all(self, server_db: Storage) -> None:
        self._seed(server_db)
        entity = server.read_graph("proj", max_observation_chars=-1)["entities"][0]
        assert obs_contents(entity) == ["aaa", "bbb", "ccc"]

    def test_search_all_projects_small_budget_trims_with_sentinel(self, server_db: Storage) -> None:
        self._seed(server_db)
        result = server.search_all_projects("foo", max_observation_chars=6)
        entity = result["results"]["proj"]["entities"][0]
        assert obs_contents(entity) == ["aaa", "bbb"]

    def test_search_all_projects_negative_returns_all(self, server_db: Storage) -> None:
        self._seed(server_db)
...
class TestGraphToolsObservationBudget:
    ...
    def _related(self, result: dict[str, object]) -> dict[str, object]:
        related = result["relatedEntities"]
        assert isinstance(related, list)
        return next(e for e in related if isinstance(e, dict))
    ...
    def test_get_entity_with_relations_small_budget_trims_both(self, server_db: Storage) -> None:
        ...
        assert obs_contents(self._related(result)) == ["xxx", "yyy"]

    def test_get_entity_with_relations_zero_keeps_only_top_both(self, server_db: Storage) -> None:
        self._seed(server_db)
    ...
    def test_get_entity_with_relations_filtered_small_budget_trims_both(self, server_db: Storage) -> None:
        self._seed(server_db)
        result = server.get_entity_with_relations("proj", "a", entityType="project", max_observation_chars=6)
        assert obs_contents(result["entity"]) == ["aaa", "bbb"]
        assert obs_contents(self._related(result)) == ["xxx", "yyy"]


class TestImplicitUsefulnessAutoVote:
    def test_search_then_edit_raises_vote_score_by_one(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "task/a", "entityType": "task", "observations": ["needle"]}])

        server.search_nodes("proj", "needle")
        server.add_observations("proj", "task/a", ["follow-up"])

        assert server_db.reads.get_entity("proj", "task/a").vote_score == 1

    def test_edit_without_prior_search_does_not_vote(self, server_db: Storage) -> None:
        server_db.entities.create("proj", [{"name": "task/a", "entityType": "task", "observations": ["needle"]}])
