...
When first interacting with a workspace, verify the `project/` entity name in memory matches the actual package/repo name. If no entity exists, create one. If the name is wrong (e.g. from a rename), rename it in place with `bulk_rename_entity(project, old_name, new_name)` (it preserves all observations and relations - no migrate-and-delete needed).
...
- **Upvote the entity/observation you create or update as a direct result of a user's explicit ask** (`vote_entity(project, name, 1)` or `vote_observation(...)`) - a direct instruction carries more weight than an incidental note, so give it a head start in ranking.
- **Vote on individual observations, not just whole entities.** `vote_observation(project, entityName, observation, vote)` casts a `+1`/`-1` on a single observation, addressed by its exact content (the same way `delete_observations` identifies one) or, preferred, by its `content_hash` (read directly off the observation object in tool output) - the hash avoids pasting the full content back and is cheaper. Upvoted observations surface first within their entity and downvoted ones sink, so a large entity leads with its most useful lines. Choose the level by what is actually good or bad:
